...
model_name = "duf"  # Out of dgsmp, mst, dwmt, padut, duf, dauhst
name = f"testing_reconstruction_single_{model_name}_{datetime.datetime.now().strftime('%Y-%m-%d_%H:%M:%S')}"
optics_model_file_path = "./system_single.yml"

log_dir = 'tb_logs'

train = False
run_on_cpu = torch.cuda.is_available() # Set to True if you prefer to run it on cpu

reconstruction_checkpoint = f"./checkpoints/checkpoints_{model_name}/best-checkpoint_single_{model_name}.ckpt"

oversample = 4
nb_rays = 20

rays_file = "./rays_single.pt"
valid_file = "./rays_valid_single.pt"
...
checkpoint_callback = ModelCheckpoint(
    ...
    filename=f'best-checkpoint_single_{model_name}',  # Checkpoint file name
