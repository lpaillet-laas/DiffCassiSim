...
data_dir_test = "./datasets_reconstruction/mst_datasets/TSA_simu_data/Truth/" # Folder where the test dataset is

torch.set_float32_matmul_precision('high')

datamodule = CubesDataModule(data_dir, data_dir_test, batch_size=1, crop_size = 512, num_workers=10)

system_name = "single"  # Out of amici, amicimis, single, singlemis
model_name = "padut"  # Out of dgsmp, mst, dwmt, padut, duf, dauhst
name = f"testing_simple_reconstruction_single_{model_name}_{datetime.datetime.now().strftime('%Y-%m-%d_%H:%M:%S')}"
optics_model_file_path = f"../system_specs/system_{system_name}.yml"
...
reconstruction_checkpoint = f"./checkpoints/checkpoints_{model_name}/best-checkpoint_simple_{system_name}_{model_name}.ckpt"

oversample = 4
nb_rays = 20

rays_file = f"./rays/rays_{system_name}.pt"
valid_file = f"./rays/rays_valid_{system_name}.pt"
...
checkpoint_callback = ModelCheckpoint(
    ...
    filename=f'best-checkpoint_simple_{system_name}_{model_name}',  # Checkpoint file name
