...
# def shift_back(inputs, dispersion_pixels): 
#     """
...
#     rounded_disp = (dispersion_pixels).round().int()
#     # Pixels to indices
#     rounded_disp_max = rounded_disp.max(dim=0).values
#     rounded_disp = rounded_disp - rounded_disp.min(dim=0).values
#     rounded_disp = rounded_disp//down_sample
#     rounded_disp = rounded_disp - (rounded_disp.max(dim=0).values - rounded_disp_max//down_sample)
...
def shift_back(inputs, dispersion_pixels, n_lambda = 28):
    """
        Input [bs, n_wav, H, W + +-||disp||]
        Output [bs, n_wav, H, W]
    """
    bs, nC, H, W = inputs.shape  
    down_sample = nC//n_lambda
    normed_disp = (torch.norm(dispersion_pixels, dim=1)*torch.sign(dispersion_pixels[:, 1])).round().int() # Pixels to indices
    normed_disp_max = normed_disp.max()
    normed_disp = normed_disp - normed_disp.min()
    normed_disp = normed_disp//down_sample
    normed_disp = normed_disp - (normed_disp.max() - normed_disp_max//down_sample)

    normed_disp = torch.nn.functional.interpolate(normed_disp.unsqueeze(0).unsqueeze(0).float(),
                                                    size=(normed_disp.shape[0]*down_sample), mode='linear').squeeze().round().int()
    max_min = normed_disp.max() - normed_disp.min()
    abs_shift = normed_disp - normed_disp.min()

    output = torch.zeros(bs, nC, H, W - max_min, device=inputs.device).float() # [bs, n_lambda, H, W]

    for i in range(nC):
        output[:, i, :, :] = inputs[:, i, :, abs_shift[i]: W - max_min + abs_shift[i]]
    
    return output

class MaskGuidedMechanism(nn.Module):
    def __init__(
            self, n_feat, dispersion_pixels):
    ...
    def forward(self, mask_shift):
        ...
        mask_emb = shift_back(mask_shift, self.dispersion_pixels)
        return mask_emb
...
class MS_MSA(nn.Module):
    def __init__(
        ...
        self.mm = MaskGuidedMechanism(dim, dispersion_pixels)
        self.dim = dim
    ...
    def __init__(
        ...
        for _ in range(num_blocks):
            self.blocks.append(nn.ModuleList([
                MS_MSA(dim=dim, dim_head=dim_head, heads=heads, dispersion_pixels=dispersion_pixels),
...
class MST(nn.Module):
    def __init__(self, dim=28, stage=3, num_blocks=[2,2,2], dispersion_pixels=None):
        ...
        for i in range(stage):
            self.encoder_layers.append(nn.ModuleList([
                MSAB(
                    dim=dim_stage, num_blocks=num_blocks[i], dim_head=dim, heads=dim_stage // dim, dispersion_pixels=self.dispersion_pixels),
        ...
        self.bottleneck = MSAB(
            dim=dim_stage, dim_head=dim, heads=dim_stage // dim, num_blocks=num_blocks[-1], dispersion_pixels=self.dispersion_pixels)
        ...
        for i in range(stage):
            self.decoder_layers.append(nn.ModuleList([
                ...
                MSAB(
                    ...
                    heads=(dim_stage // 2) // dim, dispersion_pixels=self.dispersion_pixels),
            ]))
            dim_stage //= 2
