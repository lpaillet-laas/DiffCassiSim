...
@contextmanager
def RngPooperContext(value: Union[RngPooper, None]):
...
class Torchish:
  ...
  def dtype(self) -> torch.dtype:
    #if self.value.dtype == jnp.int32 or self.value.dtype == jnp.int64:
    #  return self.value.dtype
    return j2t_dtype(self.value.dtype)
  @property
  def ndim(self) -> int: return len(self.value.shape)
  @property
  def shape(self): return self.value.shape
  @property
  def device(self) -> torch.device: return str(self.value.devices())
  # fmt: on
  ...
  def __getitem__(self, key):
    if isinstance(key, Torchish):
      key = key.value
    elif isinstance(key, tuple):
      key = tuple(k.value if isinstance(k, Torchish) else k for k in key)
    return Torchish(self.value[key])
  ...
  def __setitem__(self, key, value):
    ...
    new_value = self.value.at[key].set(value)
  ...
  def __invert__(self): return Torchish(~self.value)
  ...
  def view(self, *shape):
    return Torchish(jnp.reshape(self.value, shape[0] if len(shape) == 1 else shape))
  reshape = view
  def to(self, *args, **kwargs): return self
  def cpu(self): return self
  def numpy(self): return self
  def rot90(self, k=1, dims=(0, 1)): return Torchish(jnp.rot90(self.value, k=k, axes=tuple(dims)))
  def max(self, dim=None, axis=None, keepdim=False):
    if dim is None and axis is not None:
      dim = axis
    return Torchish(jnp.max(self.value, axis=dim, keepdims=keepdim))
  def float(self): return Torchish(self.value.astype(jnp.float32))
  ...
  def all(self, dim=None, keepdim=False): return Torchish(jnp.all(self.value, axis=dim, keepdims=keepdim))
  def unsqueeze(self, dim=0): return Torchish(jnp.expand_dims(self.value, axis=dim))
  def squeeze(self, dim=None): return Torchish(jnp.squeeze(self.value, axis=dim))
  def any(self, dim=None, keepdim=False): return Torchish(jnp.any(self.value, axis=dim, keepdims=keepdim))
  def flip(self, dims=0): return Torchish(jnp.flip(self.value, axis=dims))
  def cond(self, true_fn, false_fn, *operands):
    return jax.lax.cond(self.value, true_fn, false_fn, *operands)
  # fmt: on
...
def _coerce(x):
  ...
  if isinstance(x, Torchish):
    return x.value
  elif isinstance(x, int) or isinstance(x, float) or isinstance(x, jnp.ndarray):
    return x
  elif isinstance(x, np.ndarray):
    return jnp.array(x)  # Convert numpy arrays to JAX arrays.
  elif isinstance(x, torch.Tensor):
    return t2j_array(x)  # Convert PyTorch tensors to JAX arrays.
  else:
...
def implements(torch_function, Torchishify_output=True):
  ...
  def decorator(func):
    if Torchishify_output:
      func1 = (lambda *args, **kwargs: Torchish(func(*args, **kwargs)) if not (isinstance(func(*args, **kwargs),  tuple) or isinstance(func(*args, **kwargs),  list)) else tuple(Torchish(x) for x in func(*args, **kwargs)))
    else:
      func1 = func
...
auto_implements(torch.deg2rad, jnp.deg2rad)
auto_implements(torch.sin, jnp.sin)
auto_implements(torch.cos, jnp.cos)
auto_implements(torch.squeeze, jnp.squeeze)
auto_implements(torch.unsqueeze, jnp.expand_dims)
auto_implements(torch.sign, jnp.sign)
auto_implements(torch.square, jnp.square)
auto_implements(torch.floor, jnp.floor)

@implements(torch.full_like)
def full_like(input, fill_value, dtype=None, device=None, requires_grad=False):
  """Create a tensor with the same shape as `input`, filled with `fill_value`."""
  dtype = t2j_dtype(dtype or input.dtype)
  return jnp.full_like(_v(input), _coerce(fill_value), dtype=dtype)

@implements(torch.reshape)
def reshape(input, shape, order="C", out=None):
  """Reshape a tensor to the specified shape."""
  assert out is None, "TODO: implement `out`"
  assert order == "C", "Only 'C' order is supported"
  if isinstance(shape, Torchish):
    shape = shape.value
  return jnp.reshape(_v(input), shape)

@implements(torch.meshgrid)
def meshgrid(*tensors, indexing="xy"):
  """Create a meshgrid from the input tensors."""
  assert indexing in ("xy", "ij"), "indexing must be either 'xy' or 'ij'"
  return jnp.meshgrid(*[_v(t) for t in tensors], indexing=indexing)
    

@implements(torch.clamp)
def clamp(input, min=None, max=None, out=None):
  """Clamp all elements in the input tensor to be within [min, max]."""
  assert out is None, "TODO: implement `out`"

  if min is None and max is None:
    raise ValueError("At least one of `min` or `max` must be specified.")
  # if min is not None and max is not None and _coerce(min) > _coerce(max):
  #   raise ValueError("`min` cannot be greater than `max`.")

  input = _v(input)
  if min is not None:
    input = jnp.maximum(input, _coerce(min))
  if max is not None:
    input = jnp.minimum(input, _coerce(max))
  return input

@implements(nnF.normalize)
def normalize(input, p=2, dim=None, eps=1e-12, out=None):
  """Normalize a tensor along a specified dimension."""
  assert out is None, "TODO: implement `out`"
  assert p == 2, "Only L2 normalization is supported"
  assert dim is not None, "dim must be specified for normalization"
  
  norm = jnp.linalg.norm(_v(input), ord=p, axis=dim, keepdims=True)
  norm = jnp.maximum(norm, eps)  # Avoid division by zero
  return _coerce(input) / norm

@implements(torch.where)
def where(condition, x, y):
  """Return elements selected from `x` or `y`, depending on `condition`.

  This is a direct translation of the JAX `jnp.where` function.
  """
  return jnp.where(_coerce(condition), _coerce(x), _coerce(y))

@implements(np.hstack)
def hstack(tensors):
  return jnp.hstack(tensors)

@implements(torch.linspace)
def linspace(start, end, steps=100, out=None, dtype=None, device=None):
  assert out is None, "TODO: implement `out`"
  dtype = t2j_dtype(dtype or torch.get_default_dtype())
  assert steps > 0, "steps must be positive"
  return jnp.linspace(start=_coerce(start), stop=_coerce(end), num=steps, dtype=dtype)

@implements(torch.stack)
def stack(tensors, dim=0):
  """Stack a sequence of tensors along a new dimension."""
  # NOTE: torch.stack accepts a sequence of tensors, but we don't care about that.
...
@implements(torch.eye)
def eye(n, m=None, out=None, dtype=None, device=None, requires_grad=False):
  """Returns a 2-D tensor with ones on the diagonal and zeros elsewhere."""
  if m is None:
    m = n
  return jnp.eye(n, m, dtype=t2j_dtype(dtype or torch.get_default_dtype()))
...
def torchishify(value):
  if isinstance(value, jnp.ndarray) or isinstance(value, int) or isinstance(value, float):
    return Torchish(value)
  else:
    return value

def t2j_function(f):
  def f_jax(*args, rng=None):
    torch_args = jax.tree_util.tree_map(torchishify, args)
    ...
    elif (not isinstance(out, Torchish)) and isinstance(out, list):
...
def t2j_dtype(dtype):
  if dtype is None:
    return t2j_dtype(torch.get_default_dtype())
