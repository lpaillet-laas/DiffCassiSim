...
class FFTInteraction_N(nn.Module):
    ...
    def forward(self,x,x_enc,x_dec):
        x_enc = torch.fft.rfft2(x_enc.float(), norm='backward')
        x_dec = torch.fft.rfft2(x_dec.float(), norm='backward')
        ...
        x_recom = torch.fft.irfft2(x_recom.float()).to(x.dtype)
...
class MPRBlock(nn.Module):
    def __init__(self, n_feat=80,n_depth = 3, dispersion_pixels = [0, 0]):
        ...
        self.dispersion_pixels = torch.tensor(dispersion_pixels).float() if not isinstance(dispersion_pixels, torch.Tensor) else dispersion_pixels
        self.gdm = DegraBlock()
    ...
    def forward(self, stage_img, y,f_encoder,f_decoder,Phi,Phi_s):
        ...
        x_k_1=shift_3d(crop[:,:,:w,:Phi.shape[-1]], self.dispersion_pixels)
        # compute r_k
        r_k = self.gdm(x_k_1,y,Phi,Phi_s)
        
        r_k = shift_back_3d(r_k, self.dispersion_pixels)
...
class PADUT(nn.Module):
    def __init__(self, in_c=3,  n_feat=32,  nums_stages=5,n_depth=3, dispersion_pixels=[0, 0], mapping_cube=None):
        ...
        self.stage_model= nn.ModuleList([MPRBlock(
                n_feat=n_feat,n_depth=n_depth, dispersion_pixels=self.dispersion_pixels
            ) for _ in range(self.nums_stages)])
    ...
    def initial(self, y, Phi):
        ...
        return y_shift, z
        """ bs,row,col = y.shape
        y_shift = torch.zeros(bs, nC, row, col).float()#.cuda()
        for i in range(nC):
    ...
    def forward(self, y,input_mask=None):
        ...
        y_shift, x_0 = self.initial(y,Phi)
        y = y_shift.sum(1)
        b, h_inp, w_inp = y.shape
        r_0 = self.gdm(x_0,y,Phi,Phi_s)
        r_0 = shift_back_3d(r_0, self.dispersion_pixels)
