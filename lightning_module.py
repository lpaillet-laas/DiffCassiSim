...
class ReconstructionCASSI(L.LightningModule):
    ...
    def __init__(self, net_model_name, optics_model_file_path, rays_file = None, valid_file = None, oversample=1, nb_rays=15, mask_path = None, log_dir="tb_logs", reconstruction_checkpoint=None, train=True):
        ...
        """ if "wrong" in log_dir:
            amici_wrongoptics = HSSystem(config_file_path = "./system_amici.yml")
            self.pixel_dispersed_28 = amici_wrongoptics.central_positions_wavelengths(torch.linspace(450, 650, 28))[1]
            self.single_pixel_dispersed_28 = self.optics.central_positions_wavelengths(torch.linspace(450, 650, 28))[1]
            del amici_wrongoptics
        else: """
        ...
        if "straight_simple" in log_dir:
            try:
                self.mapping_cube = torch.load(f"./mapping_cube_straight_simple_{optics_model_file_path.split('_')[-1][:-4]}.pt", map_location=self.device)
            except:
                self.mapping_cube = self.optics.create_simple_mapping(torch.linspace(450, 650, 28), shape_scene = [512, 512], remove_y = True)
                torch.save(self.mapping_cube, f"./mapping_cube_straight_simple_{optics_model_file_path.split('_')[-1][:-4]}.pt")
        elif "simple" in log_dir:
            try:
                self.mapping_cube = torch.load(f"./mapping_cube_simple_{optics_model_file_path.split('_')[-1][:-4]}.pt", map_location=self.device)
            except:
                self.mapping_cube = self.optics.create_simple_mapping(torch.linspace(450, 650, 28), shape_scene = [512, 512])
                torch.save(self.mapping_cube, f"./mapping_cube_simple_{optics_model_file_path.split('_')[-1][:-4]}.pt")
        elif "wrong" in log_dir:
            if "mis" in log_dir:
                try:
                    self.mapping_cube = torch.load(f"./mapping_cube_amiciwrongmis.pt", map_location=self.device)
                except:
                    wrongoptics = HSSystem(config_file_path='./system_amiciwrongmis.yml')
                    self.mapping_cube = wrongoptics.get_mapping_scene_detector(torch.linspace(450, 650, 28), shape_scene = [512, 512])
                    del wrongoptics
                    torch.save(self.mapping_cube, "./mapping_cube_amiciwrongmis.pt")
                
                singleoptics = HSSystem(config_file_path='./system_singlemis.yml')
            else:
                try:
                    self.mapping_cube = torch.load(f"./mapping_cube_amiciwrong.pt", map_location=self.device)

                    """ #self.mapping_cube[:,:,:,0] = self.mapping_cube[:,:,:,0] + self.optics.system[0].film_size[1] - 512
                    #self.mapping_cube[:,:,:,1] = self.mapping_cube[:,:,:,1] + self.optics.system[0].film_size[0] - 612
                    self.mapping_cube[:,:,:,0] = self.mapping_cube[:,:,:,0]*self.optics.system[0].film_size[1]/512.
                    self.mapping_cube[:,:,:,1] = self.mapping_cube[:,:,:,1]*self.optics.system[0].film_size[0]/612.
                    #self.mapping_cube = torch.load(f"./mapping_cube_{optics_model_file_path.split('_')[-1][:-4]}.pt", map_location=self.device) """
                except:
                    wrongoptics = HSSystem(config_file_path='./system_amiciwrong.yml')
                    self.mapping_cube = wrongoptics.get_mapping_scene_detector(torch.linspace(450, 650, 28), shape_scene = [512, 512])
                    del wrongoptics
                    torch.save(self.mapping_cube, "./mapping_cube_amiciwrong.pt")
                singleoptics = HSSystem(config_file_path='./system_single.yml')
            ...
            try:
                ...
                torch.save(self.mapping_cube, f"./mapping_cube_{optics_model_file_path.split('_')[-1][:-4]}.pt")
        ...
        if self.net_model_name == 'dgsmp':
            ...
            #self.mask_model = shift(self.mask[:256, 256:].unsqueeze(0).repeat(28,1,1).unsqueeze(0), self.pixel_dispersed_28)
        ...
        try:
            ...
            torch.save(self.ray_pos, f"rays_{optics_model_file_path.split('_')[-1][:-4]}.pt")
            torch.save(self.ray_valid, f"rays_valid_{optics_model_file_path.split('_')[-1][:-4]}.pt")

        try:
            self.airy = torch.load(f"./airy_{optics_model_file_path.split('_')[-1][:-4]}.pt", map_location=self.device).half()
            ...
            torch.save(self.airy, f"./airy_{optics_model_file_path.split('_')[-1][:-4]}.pt")
    ...
    def on_predict_start(self,stage=None):
        ...
        """ try:
            file_name = f"predict_results_{self.optics_model_file_path.split('_')[-1][:-4]}.yml"
    ...
    def forward(self, x):
        ...
        #TODO things to self.mask depending on the model
    ...
    def _common_step(self, batch, batch_idx):
        ...
        # faire l'acquisition (les acquisitions si batch_size > 1) et mettre dans batch_acq (de taille batch_size, 256 + H, 256 + W)
        #TODO l'acquisition doit être flip en fonction des systèmes optiques
        ...
        """ print("uv device: ", self.ray_pos.device)
        print("texture device: ", texture.device)
        print("mask device: ", self.ray_valid.device)
        print("wavelengths device: ", self.optics.wavelengths.device)
        print("hssystem device: ", self.optics.device) """

        # batch_rendered_cube = self.optics.render_batch_based_on_saved_pos(big_uv = self.ray_pos,
        ...
        # batch_rendered_cube /= self.oversample

        # if "amici" or "single" in self.optics_model_file_path.lower():
        #     batch_rendered_cube = batch_rendered_cube.flip(1)
        
        # batch_acq = extract_acq_from_cube_batch(batch_rendered_cube, self.pixel_dispersed_28, self.optics.middle_pos, texture.shape[1:3])
        ...
        #batch_acq = extract_acq_from_cube_batch(batch_acq, self.pixel_dispersed_28, self.optics.middle_pos, texture.shape[1:3])
    ...
    def configure_optimizers(self):
        #TODO Changes based on the models, not all models use the same optimizer (refer to articles or Cai's github)
        if "dauhst" in self.net_model_name:
            optimizer = torch.optim.Adam(self.parameters(), lr=4e-4) # Reconstruction model has a lr of 4e-4 and the optics parameters have a lr of 1e-3
            ...
            optimizer = torch.optim.Adam(self.parameters(), lr=2e-4) # Reconstruction model has a lr of 4e-4 and the optics parameters have a lr of 1e-3
            return { "optimizer":optimizer,
                    "lr_scheduler":{
                    "scheduler":torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, 500, eta_min=1e-6),
                    "interval": "epoch"
